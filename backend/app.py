...
def generate_prompt(subject_data: Dict, question: str, chat_history: List[Dict[str, str]]) -> str:
    ...
    for module in subject_data['content']['modules']:
        for topic in module['topics']:
            ...
            syllabus_content.append(latex_to_plaintext(topic['content']['text'].strip()))
            ...
            for subtopic in topic.get('subtopics', []):
                ...
                syllabus_content.append(latex_to_plaintext(subtopic['content']['text'].strip()))
...
def get_file_content(file_id):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error fetching file from Google Drive")
...
def load_subject_data_from_drive(department: str, semester: str, subject: str) -> Dict:
    ...
    try:
        ...
        dept_folder = next((f for f in dept_folders if f['name'] == department and f['mimeType'] == 'application/vnd.google-apps.folder'), None)
...
async def get_semesters(request: DepartmentRequest):
    ...
    try:
        # First find the department folder
        dept_folders = list_drive_folder(GOOGLE_DRIVE_FOLDER_ID)
        dept_folder = next((f for f in dept_folders if f['name'] == request.department and f['mimeType'] == 'application/vnd.google-apps.folder'), None)
...
async def get_subjects(request: SemesterRequest):
    ...
    try:
        ...
        # Then find the semester folder
        semester_folders = list_drive_folder(dept_folder['id'])
        semester_folder = next((f for f in semester_folders if f['name'] == request.semester and f['mimeType'] == 'application/vnd.google-apps.folder'), None)
...
async def chat_with_ai(request: ChatRequest):
    ...
    try:
        ...
        # Update chat context
        context_key = get_context_key(request.department, request.semester, request.subject)
